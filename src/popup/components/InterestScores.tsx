...
const ScoreItem: React.FC<ScoreItemProps> = ({ score }) => (
	<div className="score-item">
		...
		<div className="score-details">
			<ScoreFactor
				label="滞在時間"
				weight={score.factors.timeWeight}
				className="time"
			/>
			<ScoreFactor
				label="スクロール"
				weight={score.factors.scrollWeight}
				className="scroll"
			/>
			<ScoreFactor
				label="エンゲージメント"
				weight={score.factors.engagementWeight}
				className="engagement"
			/>
		</div>
		<p className="last-updated">最終更新: {formatDate(score.lastUpdated)}</p>
	</div>
);
	...
	return (
		<div className="interest-scores">
			...
			<div className="scores-list">
				{scores.map((score) => (
					<ScoreItem key={score.domain} score={score} />
				))}
			</div>
		</div>
	);
