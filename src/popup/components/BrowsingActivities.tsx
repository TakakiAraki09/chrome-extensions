...
const ActivityItem: React.FC<ActivityItemProps> = ({ activity }) => {
	...
	const engagementRate =
		totalTime > 0 ? (activity.focusTime / totalTime) * 100 : 0;
	...
	return (
		<div className="activity-item">
			...
			<div className="activity-stats">
				<ActivityStat
					label="滞在時間"
					value={formatDuration(activity.focusTime)}
				/>
				<ActivityStat
					label="最大スクロール"
					value={activity.maxScrollDepth}
					unit="%"
				/>
				<ActivityStat
					label="スクロール距離"
					value={Math.round(activity.totalScrollDistance)}
					unit="px"
				/>
				<ActivityStat
					label="エンゲージメント率"
...
export const BrowsingActivities: React.FC<BrowsingActivitiesProps> = ({
	activities,
}) => {
		...
		<div className="browsing-activities">
			...
			<div className="activities-list">
				{activities.map((activity) => (
					<ActivityItem
						key={`${activity.url}-${activity.startTime}`}
						activity={activity}
					/>
				))}
			</div>
